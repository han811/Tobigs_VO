...
def get_data_info_tovi(folder_list, seq_len_range, overlap, sample_times=1, pad_y=False, shuffle=False, sort=True):
    X_path, Y = [], []
    X_len = []
    for folder in folder_list:
        ...
        x_segs = [fpaths[i:i+seq_len] for i in range(0, n_frames, jump)]
        y_segs = [poses[i:i+seq_len] for i in range(0, n_frames, jump)]
        Y += y_segs
        X_path += x_segs
