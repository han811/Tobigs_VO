...
if par.resume:
    ...
    temp = torch.load(par.load_optimizer_path)
    temp['param_groups'][0]['eps'] = 1e-10
...
#===================================================================================
# epoch 높인다면 % 10 정도로 바꿔야함, 이건 매 epoch마다 기록
# train의 for ep in range(par.epochs): 안에 넣어서 학습과 동시에 기록

    if ep % 1 == 0:
        summary.add_scalar('training loss', loss_mean, ep)
        summary.add_scalar('validation loss', loss_mean_valid, ep)
