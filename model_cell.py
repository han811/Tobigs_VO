...
class DeepVO(nn.Module):
    def __init__(self, imsize1, imsize2, batchNorm=True):
        ...
        self.rnn = nn.LSTMCell(
                    ...
                    #expected 30720... 왜 6이 들어가냐 자꾸
                    #이걸 6으로 바꾸면 m1: 6*30720 m2: 6*4000이라 miss match
        ...
        for m in self.modules():
            if isinstance(m, nn.Conv2d) or isinstance(m, nn.ConvTranspose2d) or isinstance(m, nn.Linear):
                ...
                kaiming_normal_(m.weight_ih)  #orthogonal_(m.weight_ih_l0)
                kaiming_normal_(m.weight_hh)
                m.bias_ih.data.zero_()
                m.bias_hh.data.zero_()
                # Set forget gate bias to 1 (remember)
                n = m.bias_hh.size(0)
                start, end = n//4, n//2
                m.bias_hh.data[start:end].fill_(1.)
    ...
    def forward(self, x): 
       ...
       # RNN
        hx = Variable(torch.zeros(par.batch_size, 1000))
        cx = Variable(torch.zeros(par.batch_size, 1000))
        print(x[0].size())
        for i in range(6):
        ...
        #for input in x:
        #  out, hc =self.rnn(input)
        #out = self.rnn_drop_out(out)
        #out = self.linear(out)

        #self.hx1, self.cx1 = self.rnn1(x, (self.hx0, self.cx0))
        #x = self.hx1      
        #self.hx2, self.cx2 = self.rnn2(x, (self.hx1, self.cx1))
        #x = self.hx2
        #out = self.linear(x)
